...
%Authors: 
%Subham Bose, Modified by Maxwell Pinz
% This code was mostly made from: 
% https://mtex-toolbox.github.io/TiBetaReconstruction.html

%this is going to be a batch processing program. 

mtexFolder = '/home/lferrei6/repositories/mtex-5.8.2/';
cd(mtexFolder);
startup_mtex;

BaseInputFolder = 'pipeline_input/';
BaseOutputFolder = 'pipeline_output/';
BetaFolder = 'betaGrains/';
samplesAnalyzed = string(['a01'; 'a03'; 'a06'; 'a07']);

% analysisFolder = pwd;
analysisFolder = '/scratch16/sghosh20/Ti64Team/preprocessing-ti64/';
cd(analysisFolder);


for ii = 1:length(samplesAnalyzed)
  sample = samplesAnalyzed(ii);
  AlphaEbsdFolder = strcat(BaseInputFolder, sample, '/');
  if exist(AlphaEbsdFolder, 'dir')
    if ~exist(BaseOutputFolder, 'dir')
      mkdir(BaseOutputFolder);
      mkdir(strcat(BaseOutputFolder, BetaFolder));
    elseif ~exist(strcat(BaseOutputFolder, BetaFolder), 'dir')
      mkdir(strcat(BaseOutputFolder, BetaFolder));
    end
    BetaEbsdFolder = strcat(BaseOutputFolder, BetaFolder, sample, '/');
    fprintf('\nAnalyzing sample: %s\n', sample);
    step02(AlphaEbsdFolder, BetaEbsdFolder);
    fprintf('\n\nFinished sample: %s\n', sample);
  else
    fprintf('\nNo EBSD data for this sample: %s\n', sample);
  end
end
...
function step02(AlphaEbsdFolder, BetaEbsdFolder)
  ...
  mkdir(BetaEbsdFolder)
  BetaThreshold = 2.5;
  MinBetaSizePixels = 15;

  FileList  = dir(strcat(AlphaEbsdFolder,'*.ang'));
  nFiles = length(FileList); 

  AlphaImageFolder = ''; % this should be user specified, basically making .ipf images 

  ShouldPlotReconstructionSteps = 1;
  ShouldPlotFinalReconstructionOnly = 1;
  %ReconstructionPlotFolder = 'ReconstructedBetaPlots_Downsampled3/';
  ReconstructionPlotFolder = strcat(BetaEbsdFolder,'BetaGrainImages/');
  mkdir(ReconstructionPlotFolder)
  %%%% import Alpha .ang file



  for iFile = 1:nFiles

    AlphaEbsdFile = strcat(AlphaEbsdFolder,FileList(iFile).name);
    LocalEbsdBaseName = erase(FileList(iFile).name,'.ang');
    AlphaEBSD= EBSD.load(AlphaEbsdFile);
